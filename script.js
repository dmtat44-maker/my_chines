...
function interactiveSceneConfig(day, rp){
  if(rp.key === "magic"){
    const magicConfigs = {
      1:{title:"Интерактивная сцена: дорога к замку", intro:"Маг идёт по дороге к замку. Китайское приветствие открывает первые ворота.", zone:"ворота замка", labels:["1. Поздороваться: 你好","2. Понять фразу: 我是朋友","3. Подойти к замку"], speeches:["Скажи 你好, чтобы дорога засияла.","Хорошо! Теперь пойми, кто такой 朋友.","Остался последний шаг — и маг подойдёт к замку."], success:"Маг доходит до ворот замка, и на двери появляется первый светящийся знак."},
      2:{title:"Интерактивная сцена: три камня на дороге", intro:"На дороге к замку лежат волшебные камни. Их нужно посчитать по-китайски.", zone:"каменная тропа", labels:["1. Найти число 三","2. Понять слово 我们","3. Пройти тропу"], speeches:["Посчитай камни на дороге.","Верно! Теперь помни: 我们 — это “мы”.","Ещё один ответ — и тропа откроется."], success:"Три камня загораются, и маг проходит дальше к замку."},
      3:{title:"Интерактивная сцена: синий кристалл", intro:"У замка нужен синий кристалл. Герой учит цвета, чтобы выбрать правильный.", zone:"кристалл найден", labels:["1. Найти 蓝色","2. Узнать 水晶","3. Активировать кристалл"], speeches:["Выбери правильный цвет кристалла.","Теперь запомни слово 水晶.","Осталось активировать кристалл."], success:"Синий кристалл светится, и дорога к замку становится ярче."},
      4:{title:"Интерактивная сцена: первое заклинание", intro:"Китайские глаголы помогают магу произнести простое заклинание и идти дальше.", zone:"заклинание готово", labels:["1. Найти 学","2. Найти 说","3. Произнести заклинание"], speeches:["Маг учится первому действию.","Хорошо, теперь нужно слово 说.","Последний ответ — и заклинание сработает."], success:"Маг произносит фразу, и над дорогой появляется волшебная арка."},
      5:{title:"Интерактивная сцена: отдых у фонтана", intro:"После практики магу нужно восстановить силы и выпить воды.", zone:"фонтан у замка", labels:["1. Собрать 喝 + 水","2. Узнать 练习","3. Дойти до фонтана"], speeches:["После практики нужно восстановиться.","Верно, рядом уже виден фонтан.","Последний шаг — и маг дойдёт до воды."], success:"Маг доходит до фонтана, пьёт воду и готовится идти дальше."},
      6:{title:"Интерактивная сцена: вопрос хранителя", intro:"Хранитель спрашивает, где находится маг. Нужно понять вопрос и выбрать место.", zone:"башня замка", labels:["1. Найти 哪里","2. Найти 塔","3. Ответить хранителю"], speeches:["Сначала пойми вопрос хранителя.","Хорошо, теперь найди башню.","Остался один ответ, чтобы пройти к башне."], success:"Хранитель кивает, и дверь башни открывается."},
      7:{title:"Интерактивная сцена: рассказ у ворот", intro:"Маг говорит о себе и о том, что ему нравится китайский язык.", zone:"ворота слушают", labels:["1. Найти 中文","2. Найти 喜欢","3. Сказать о себе"], speeches:["Расскажи, что тебе нравится.","Отлично, фраза почти готова.","Последний ответ — и ворота услышат героя."], success:"Ворота замка понимают фразу героя и открывают следующий проход."},
      8:{title:"Интерактивная сцена: кристалл и ключ", intro:"Чтобы открыть замок, маг ищет кристалл и ключ.", zone:"ключ найден", labels:[`1. Найти ${rp.objectHanzi || '水晶'}`,"2. Найти 钥匙","3. Открыть дверь"], speeches:["Сначала найди кристалл.","Отлично, теперь нужен ключ.","Последний ответ — и дверь откроется."], success:"Кристалл и ключ соединяются, дверь замка открывается."},
      9:{title:"Интерактивная сцена: диалог с другом", intro:"Перед входом в зал магии герой говорит короткую фразу другу.", zone:"зал магии", labels:["1. Найти 我的","2. Найти 朋友","3. Завершить диалог"], speeches:["Скажи другу важную фразу.","Хорошо, осталось слово 朋友.","Последний шаг — и диалог завершён."], success:"Друг улыбается, и маг входит в зал магии."},
      10:{title:"Интерактивная сцена: вход в замок", intro:"Финальное испытание первого блока: маг должен дойти до замка и открыть вход.", zone:"замок открыт", labels:["1. Найти 加油","2. Найти 今天","3. Войти в замок"], speeches:["Финал пути — соберись!","Отлично, замок уже рядом.","Последний ответ — и вход откроется."], success:"Маг доходит до замка, ворота открываются, первый блок завершён."}
    };
    return {...magicConfigs[day], stageClass:"magic-road", guideEmoji:"🧙", guideName:"Хранитель", destination:"🏰"};
  }
  ...
  return {...(configs[day] || {title:`Интерактивная сцена: день ${day}`, intro:`Китайский язык открывает действие ${place}.`, zone:"сцена", labels:["1. Шаг 1","2. Шаг 2","3. Шаг 3"], speeches:["Начни с первого ответа.","Теперь второй шаг.","Остался последний шаг."], success:`Герой завершает интерактив ${place}.`}), stageClass:"football-field", guideEmoji:"🧢", guideName:"Тренер", destination:""};
}
...
function interactiveStageHtml(day, data, done){
  ...
  return `
    <div class="mini-stage">
      ...
      <div class="field field-day-${day} ${h(config.stageClass || 'football-field')} stage-step-${correctCount}">
        <div class="midline"></div><div class="circle-line"></div>
        <div class="coach"><div><span>${h(config.guideEmoji || '🧢')}</span>${h(config.guideName || 'Тренер')}</div></div>
